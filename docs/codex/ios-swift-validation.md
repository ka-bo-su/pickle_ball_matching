...
Last checked: 2026-06-01 05:03 JST

| Command | Result | Notes |
|---|---|---|
| `scripts/codex/preflight.sh` | passed | Tuist missing only; optional |
| `swift test` | passed | 8 core tests, 0 failures; local linker warning for missing Homebrew GMP search path is non-blocking |
| `swiftlint --no-cache` | passed | 0 violations in 15 files |
...
| `xcodebuild test -project PickleBallMatching.xcodeproj -scheme PickleBallMatching -destination 'platform=iOS Simulator,name=iPhone 16'` | passed | 8 core tests plus 9 app tests passed |

Full validation script result: `scripts/codex/validate-ios.sh` passed on `codex/sbi-25-participant-status`.
